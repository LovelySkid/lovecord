...
namespace LovecordInstaller
{
    ...
    public class LauncherForm : Form
    {
        ...
        private LovecordBackend _backend;

        public LauncherForm()
        {
            this.Text = "Lovecord Installer";
            ...
            var iconStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("LovecordInstaller.icon.ico");
        ...
        {
            var userDataFolder = Path.Combine(Path.GetTempPath(), "LovecordInstaller_WebView2");
            ...
            catch (Exception ex)
            {
                ...
                {
                    MessageBox.Show(
                        "Microsoft Edge WebView2 Runtime is required to run the Lovecord Installer but was not found on your system.\n\n" +
                        "Please download and install it from:\nhttps://aka.ms/webview2\n\n" +
                        "After installing, restart the Lovecord Installer.",
            ...
            _backend = new LovecordBackend(this, _webView);
            ...
            using (var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("LovecordInstaller.index.html"))
            using (var reader = new StreamReader(stream))
            {
                ...
                try {
                    using (var iconStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("LovecordInstaller.icon.ico"))
                    {
    ...
    public class LovecordBackend
    {
        ...
        const string GITEA_REPO = "Lovecord/Lovecord";
        const string GITEA_URL  = "https://source.Lovecord.st";
        const string DIST_ZIP   = "Lovecord-dist.zip";

        public LovecordBackend(LauncherForm form, WebView2 webView)
        {
            _form = form;
            _webView = webView;
            ...
            _distDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Lovecord", "dist");
        }
        ...
        public async Task<bool> IsInjected(string path)
        {
            return await Task.Run(() => {
                ...
                return Directory.Exists(appDir) && File.Exists(pkgPath) && File.ReadAllText(pkgPath).Contains("\"Lovecord\"");
            });
        }

        /// <summary>
        /// Returns true if a third-party mod (Vencord, Equicord, OpenAsar) is detected
        /// but Lovecord is NOT yet injected.
        /// </summary>
        public async Task<bool> HasThirdPartyMod(string path)
        {
            return await Task.Run(() => {
                ...
                // Already Lovecord → not a third-party mod
                if (content.Contains("\"Lovecord\"")) return false;
        ...
        private async Task EnsureDistAsync()
        {
            ...
            _http.DefaultRequestHeaders.Add("User-Agent", "Lovecord-Installer/2.0");
            ...
            {
                throw new Exception($"Could not reach source.Lovecord.st: {ex.Message}. Check your internet connection.");
            }
            ...
            var tmpZip = Path.Combine(Path.GetTempPath(), "Lovecord-dist.zip");
                ...
                using (var fs = new FileStream(tmpZip, FileMode.Create, FileAccess.Write, FileShare.None, 81920, true))
                {
                    ...
                    while ((read = await contentStream.ReadAsync(buffer, 0, buffer.Length)) > 0)
                    {
                        ...
                        if (percent - lastReportedPercent >= 0.5 || percent >= 100.0)
                        {
                            ...
                            SetProgress(overallPercent, "Downloading Lovecord...", readMB, totalMB);
                        }
                    }
                }
            }
        ...
        private void DoInject(string resPath)
        {
            ...
            SetProgress(92, "Configuring Lovecord loader...");
        ...
        private void CleanModulePatches(string resPath)
        {
            ...
            {
                Console.WriteLine($"[Lovecord] CleanModulePatches warning: {ex.Message}");
            }
        }
        ...
        private void DoUninject(string resPath)
        {
            ...
            if (Directory.Exists(appDir))
            {
                ...
                if (File.Exists(pkg) && File.ReadAllText(pkg).Contains("\"Lovecord\""))
                {
        ...
        private void WriteLoader(string appDir)
        {
            ...
            File.WriteAllText(Path.Combine(appDir, "package.json"), "{\"name\":\"Lovecord\",\"main\":\"index.js\"}");
            File.WriteAllText(Path.Combine(appDir, "index.js"),
                ...
                $"if (!fs.existsSync(patcherPath)) throw new Error('[Lovecord] patcher.js not found. Expected at: ' + primary);\n" +
                $"require(patcherPath);\n"
            );
        }
