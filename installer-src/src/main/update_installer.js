...
const getJSON = phin.defaults({
    method: "GET",
    parse: "json",
    headers: {"User-Agent": `Lovecord Installer/${version}`},
    followRedirects: true
});

/* eslint-disable no-console */
export default async function () {
    const downloadUrl = `https://source.${domain}/api/v1/repos/lovecord/lovecord/releases/latest`;
    console.info(`Lovecord Installer ${version}`);
    ...
    try {
        ...
        if (semverGreaterThan(latestVersion, version)) {
            ...
            const result = await dialog.showMessageBox({
                ...
                message: `A new version of the Lovecord installer is available. Click "Download" to download the newest version.`,
