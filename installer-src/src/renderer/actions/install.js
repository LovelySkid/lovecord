...
const RELEASE_API = `https://source.${domain}/api/v1/repos/lovecord/lovecord/releases/latest`;
const DIST_ZIP = "lovecord-dist.zip";
const distDir = path.join(process.env.LOCALAPPDATA, "Lovecord", "dist");
...
async function cleanModulePatches(resourcesPath) {
    try {
        ...
        log(`[Lovecord] CleanModulePatches warning: ${err.message}`);
    }
}
...
function downloadFileAsync(url, destPath, onProgress) {
    return new Promise((resolve, reject) => {
        ...
        https.get(url, { headers: { "User-Agent": "Lovecord-Installer/3.0" }, rejectUnauthorized: false }, (response) => {
...
const getJSON = phin.defaults({
    ...
    headers: { "User-Agent": "Lovecord-Installer/3.0", "Accept": "application/json" }
});
...
async function downloadDist() {
    ...
    try {
        ...
        lovecordVersion = release && release.tag_name;
        if (!assetUrl) {
    ...
    const tmpZip = path.join(remote.app.getPath("temp"), "lovecord-dist.zip");
    log(`Downloading Lovecord ${lovecordVersion} package...`);
    try {
        await downloadFileAsync(assetUrl, tmpZip, (percent, downloaded, total) => {
            ...
            status.set(`Downloading Lovecord... (${dlMB}/${totalMB} MB)`);
        });
...
async function writeLoader(appDir) {
    ...
    const loaderCode = `// Lovecord Injector
"use strict";
const fs = require('fs');
...
async function applyDefaultPluginsSetting() {
    try {
        const settingsDir = path.join(process.env.APPDATA, "Lovecord", "settings");
        const settingsPath = path.join(settingsDir, "settings.json");
        await fs.mkdir(settingsDir, { recursive: true });

        // Always enable default plugins by removing the 'plugins' key from settings.json
        // so that Lovecord natively loads its enabledByDefault values.
        let existing = null;
...
async function injectShims(paths) {
    ...
    for (const resPath of paths) {
        ...
        try {
            ...
            log("2. Configuring Lovecord loader...");
...
export default async function(paths) {
    try {
        ...
        await fs.mkdir(path.join(localAppData, "Lovecord"), { recursive: true });
        log("✅ Local AppData directory prepared");
        progress.set(MAKE_DIR_PROGRESS);
        lognewline("Downloading Lovecord package...");
        ...
        lognewline("Injecting Lovecord shims...");
