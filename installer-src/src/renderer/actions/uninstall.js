...
async function shouldAutoRestart() {
    try {
        const prefsPath = path.join(process.env.APPDATA, "Lovecord", "settings", "installer-prefs.json");
...
async function deleteShims(paths) {
    ...
    for (const resPath of paths) {
        log(`Removing Lovecord from: ${resPath}`);
        try {
            ...
            log(`❌ Could not remove Lovecord from ${resPath}`);
            log(`❌ ${err.message}`);
            return err;
        }
    }
...
export default async function(paths) {
    try {
        ...
        lognewline("Deleting Lovecord loader and restoring files...");
