@echo off
:: ─── Lovecord — Publier une nouvelle release sur Gitea ──────────────────────
...
if "%NOTES%"=="" set NOTES=Lovecord %VERSION%

:: ── Config Gitea ──────────────────────────────────────────────────────────────
set GITEA_URL=https://source.lovecord.gg
set GITEA_REPO=lovecord/lovecord
...
set DIST_ZIP=%OUT_DIR%\lovecord-dist.zip
set INSTALLER_EXE=%OUT_DIR%\Lovecord-Installer.exe
set VERSION_JSON=%OUT_DIR%\version.json
set DESKTOP_ASAR=dist\desktop.asar

echo.
echo  ╔═══════════════════════════════════════════════════╗
echo  ║    LOVECORD — Publication release v%VERSION%
echo  ╚═══════════════════════════════════════════════════╝
echo.
...
:: ── 5. Lovecord-Installer.exe ────────────────────────────────────────────────
echo.
echo  [5/8] Compilation de Lovecord-Installer.exe...
...
if not exist "%INSTALLER_EXE%" (
    echo  [ERREUR] Lovecord-Installer.exe introuvable apres compilation.
    pause
    exit /b 1
)
for %%F in ("%INSTALLER_EXE%") do echo  [5/8] Lovecord-Installer.exe cree (%%~zF octets)

:: ── 6. lovecord-dist.zip ─────────────────────────────────────────────────────
echo.
echo  [6/8] Creation de lovecord-dist.zip...
...
powershell -NoProfile -Command "Add-Type -Assembly System.IO.Compression.FileSystem; $src = (Resolve-Path '%DIST_DIR%').Path; $dst = (Join-Path (Resolve-Path 'release\installer').Path 'lovecord-dist.zip'); [System.IO.Compression.ZipFile]::CreateFromDirectory($src, $dst, [System.IO.Compression.CompressionLevel]::Optimal, $false)"
if not exist "%DIST_ZIP%" (
    echo  [ERREUR] Impossible de creer lovecord-dist.zip
    pause
    exit /b 1
)
for %%F in ("%DIST_ZIP%") do echo  [6/8] lovecord-dist.zip cree (%%~zF octets)
...
(
    ...
    echo   "installerUrl": "%GITEA_URL%/%GITEA_REPO%/releases/download/v%VERSION%/Lovecord-Installer.exe",
    echo   "distUrl": "%GITEA_URL%/%GITEA_REPO%/releases/download/v%VERSION%/lovecord-dist.zip",
...
:: 8c. Upload — Lovecord-Installer.exe (via curl, < 100MB)
echo  Upload de Lovecord-Installer.exe...
curl -s -X POST "%GITEA_API%/repos/%GITEA_REPO%/releases/%RELEASE_ID%/assets?name=Lovecord-Installer.exe" ^
    -H "Authorization: token %GITEA_TOKEN%" ^
    -H "Content-Type: application/octet-stream" ^
    --data-binary "@%INSTALLER_EXE%" >nul
if errorlevel 1 ( echo  [ERREUR] Upload Lovecord-Installer.exe echoue. & pause & exit /b 1 )

:: 8d. Upload — lovecord-dist.zip (via curl, < 100MB)
echo  Upload de lovecord-dist.zip...
curl -s -X POST "%GITEA_API%/repos/%GITEA_REPO%/releases/%RELEASE_ID%/assets?name=lovecord-dist.zip" ^
    -H "Authorization: token %GITEA_TOKEN%" ^
    -H "Content-Type: application/zip" ^
    --data-binary "@%DIST_ZIP%" >nul
if errorlevel 1 ( echo  [ERREUR] Upload lovecord-dist.zip echoue. & pause & exit /b 1 )
...
powershell -NoProfile -Command ^
    ...
    "$uri = 'https://source.lovecord.gg/api/v1/repos/lovecord/lovecord/releases/%RELEASE_ID%/assets?name=desktop.asar';" ^
...
echo.
echo  ╔═══════════════════════════════════════════════════════════════════════╗
echo  ║  Lovecord v%VERSION% publie avec succes sur Gitea !
echo  ║
echo  ║  URL : %GITEA_URL%/%GITEA_REPO%/releases/tag/v%VERSION%
echo  ║
echo  ║  Fichiers publies :
echo  ║    Lovecord-Installer.exe    — installeur .exe avec GUI
echo  ║    lovecord-dist.zip         — JS obfusques (pour l'injec.)
