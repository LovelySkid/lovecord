/*
 * Lovecord â€” Installer via EquilotlCli
 * TÃ©lÃ©charge EquilotlCli.exe depuis les releases Equicord et le lance
 * avec les variables d'environnement pointant vers les fichiers Lovecord.
 *
 * L'exe affiche une interface graphique permettant de choisir le Discord cible.
 *
 * Usage:
 *   pnpm inject    â†’ installe Lovecord dans le Discord choisi
 *   pnpm uninject  â†’ dÃ©sinstalle Lovecord du Discord choisi
...
async function ensureBinary() {
    ...
    if (existsSync(outputFile)) {
        console.log("[Lovecord] Installer already present, using local copy.");
        return outputFile;
    }

    console.log("[Lovecord] Downloading installer (" + filename + ")...");

    const res = await fetch(BASE_URL + filename, {
        headers: { "User-Agent": "Lovecord (https://github.com/lovecord/lovecord)" }
    });
    ...
    console.log("[Lovecord] Installer downloaded successfully!");
    return outputFile;
}
...
function checkBuild() {
    ...
    if (!existsSync(patcherPath)) {
        console.error("\x1b[31m[Lovecord] dist/desktop/patcher.js not found!\x1b[0m");
        ...
        for (const ver of versions) {
            ...
            if (existsSync(join(base, ver)) && !existsSync(appAsarPath) && !existsSync(backupPath)) {
                try {
                    ...
                    console.log(`[Lovecord] Supprimé le dossier de mise à jour Discord incomplet : ${join(base, ver)}`);
                } catch (e) {
                    console.warn(`[Lovecord] Impossible de supprimer ${join(base, ver)}: ${e.message}`);
                }
            }
        }
    }
...
function cleanOldLovecord(isUninstall) {
    console.log("[Lovecord] Cleaning previous installations...");
    ...
    for (const resourcesDir of candidates) {
        ...
        try {
            if (existsSync(appDirPath)) {
                ...
                try {
                    ...
                    if (existsSync(pkgFile)) {
                        ...
                        if (pkg.name === "lovecord") shouldDelete = true;
                ...
                if (shouldDelete) {
                    ...
                    console.log(`[Lovecord] Removed legacy app/ folder in ${resourcesDir}`);
                    cleanedAny = true;
                }
            }
            ...
            if (!isUninstall && existsSync(backupPath)) {
                ...
                console.log(`[Lovecord] Restored _app.asar â†’ app.asar in ${resourcesDir}`);
                cleanedAny = true;
            }

        } catch (e) {
            console.error(`[Lovecord] Error cleaning ${resourcesDir}:`, e.message);
        }
    }

    if (cleanedAny) {
        console.log("[Lovecord] Cleanup done.");
    } else {
        console.log("[Lovecord] Nothing to clean.");
    }
}
        ...
        for (const ver of versions) {
            ...
            if (existsSync(backupPath)) {
                ...
                if (existsSync(updateExe)) {
                    console.log(`[Lovecord] Launching ${channel}...`);
                    ...
                    if (existsSync(directExe)) {
                        console.log(`[Lovecord] Launching ${channel} (direct)...`);
                        exec(`"${directExe}"`);
                    }
                }
...
cleanOldLovecord(isUninstall);
if (!isUninstall) checkBuild();

const installerBin = await ensureBinary();

console.log("[Lovecord] Injecting...");
...
try {
    execFileSync(installerBin, mappedArgs, {
        ...
        env: {
            ...
            LOVECORD_DIRECTORY: join(BASE_DIR, "dist", "desktop")
        }
    });
} catch {
    console.error("[Lovecord] Injection failed.");
    process.exit(1);
}
