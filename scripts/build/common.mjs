...
export const gitHash = process.env.LOVECORD_HASH || execSync("git rev-parse HEAD", { encoding: "utf-8" }).trim();
...
export const globPlugins = kind => ({
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "import-plugins" }, async () => {
            const pluginDirs = ["plugins/_api", "plugins/_core", "plugins", "userplugins", "lovecordplugins", "lovecordplugins/_api"];
            ...
            // ─── External User Plugins (~/Documents/Lovecord/userplugins/) ───────────
            // Scan and auto-create the external userplugins directory.
            const externalUserPluginsDir = join(homedir(), "Documents", "Lovecord", "userplugins");
            try {
...
export const gitRemotePlugin = {
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "git-remote" }, async () => {
            let remote = process.env.LOVECORD_REMOTE;
            if (!remote) {
...
export const commonOpts = {
    ...
    alias: {
        ...
        "@lovecordplugins": "./src/lovecordplugins",
    }
};
