...
(async () => {
    ...
    // Setup userplugins
    try {
        const userTarget = join(homedir(), "Documents", "Lovecord", "userplugins");
        ...
        console.error("[Lovecord] Failed to setup userplugins link", e);
    }
    ...
    const lovecordFlag = args.includes("--lovecord");

    let dirs: string[];

    if (lovecordFlag) {
        dirs = ["src/lovecordplugins"];
    } else if (equicordFlag) {
        dirs = ["src/lovecordplugins", "src/userplugins"];
    } else if (vencordFlag) {
        dirs = ["src/plugins", "src/plugins/_core"];
    } else {
        dirs = ["src/plugins", "src/plugins/_core", "src/lovecordplugins", "src/userplugins"];
    }
