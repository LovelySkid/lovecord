/*
 * Lovecord — Local un-injector for Discord Desktop
...
function findAllDiscordResources() {
    ...
    // Filtrer uniquement les paths avec une injection Lovecord présente
...
function uninject(resourcesDir) {
    ...
    // Vérifier que le dossier app/ a bien été créé par Lovecord
    if (existsSync(appDirPath)) {
        try {
            if (existsSync(join(appDirPath, "index.js"))) {
                ...
                if (!indexContent.includes("Lovecord Injector") && !indexContent.includes("Lovecord")) {
                    console.warn(`\x1b[33m[Lovecord] Le dossier app/ existe mais n'a pas l'air d'avoir été créé par Lovecord.\x1b[0m`);
        ...
        console.log("[Lovecord] Suppression du dossier app/ injecté...");
        rmSync(appDirPath, { recursive: true, force: true });
    } else {
        console.log("\x1b[33m[Lovecord] Aucun dossier app/ injecté trouvé.\x1b[0m");
    }

    // Restaurer le backup
    if (existsSync(backupPath) && !existsSync(appAsarPath)) {
        console.log("[Lovecord] Restauration _app.asar → app.asar...");
        renameSync(backupPath, appAsarPath);
    } else if (existsSync(backupPath) && existsSync(appAsarPath)) {
        console.log("[Lovecord] app.asar déjà présent, nettoyage du backup...");
        rmSync(backupPath, { force: true });
    }

    console.log(`\x1b[32m[Lovecord] Désinjection réussie depuis : ${resourcesDir}\x1b[0m`);
...
if (allResources.length === 0) {
    console.error("\x1b[31m[Lovecord] Aucune installation Discord avec Lovecord injecté trouvée.\x1b[0m");
    console.error("\x1b[33m           Assurez-vous que Lovecord a bien été injecté via 'pnpm inject'.\x1b[0m");
    process.exit(1);
}
...
if (uninjectCount === 0) {
    console.error("\x1b[31m[Lovecord] Aucune désinjection réussie.\x1b[0m");
    process.exit(1);
}

console.log(`\n\x1b[32m[Lovecord] ${uninjectCount}/${allResources.length} désinjection(s) réussie(s).\x1b[0m`);
