...
// ─── Configuration de Build Lovecord ─────────────────────────────────────────

function killLovecord() {
    const releaseDir = join(__dirname, "release", "lovecord-dist");
...
function buildLovecordFromDiscord(discordApp) {
    ...
    console.log("[lovecord] Préparation de _app.asar...");
...
app.setPath("userData", path.join(app.getPath("appData"), "Lovecord"));
...
`);
    ...
    writeFileSync(join(outApp, "package.json"), JSON.stringify({ name: "lovecord", main: "index.js", version: "1.18.0" }, null, 2));
...
`);
    ...
    const lovecordPreload = join(__dirname, "lovecord-preload.js");
    if (existsSync(lovecordPreload)) {
        cpSync(lovecordPreload, join(outDist, "preload.js"));
    }
    ...
    if (existsSync(macCursorsSrc)) {
        console.log("[lovecord] Copie des curseurs macOS...");
        cpSync(macCursorsSrc, join(outRes, "mac"), { recursive: true });
    } else {
        console.warn("[lovecord] ATTENTION: dossier 'mac' introuvable a la racine, le plugin CursorMacOS ne fonctionnera pas dans le build package.");
    }
    ...
    const iconSrc = join(__dirname, "lovecord.ico");
    if (existsSync(iconSrc)) {
        ...
        try {
            ...
            if (existsSync(rcedit)) {
                execSync(`"${rcedit}" "${discordExe}" --set-icon "${iconSrc}" --set-version-string "ProductName" "Lovecord" --set-version-string "FileDescription" "Lovecord"`, { stdio: "ignore" });
            }
        } catch (e) { }
    }

    console.log(`[lovecord] Build terminé -> ${outDir}`);
}
...
buildLovecordFromDiscord(discord);

module.exports = {
    appId: "com.lovecord.app",
    productName: "Lovecord",
    copyright: "Copyright 2026 Lovecord",
