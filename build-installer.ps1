# build-installer.ps1 — Build Lovecord-Installer.exe (Electron + electron-builder)
...
$OutExe    = Join-Path $OutDir "Lovecord-Installer.exe"

Write-Host ""
Write-Host "  [Lovecord] Building Electron installer..." -ForegroundColor Cyan
...
if (Test-Path $OutExe) {
    ...
    Write-Host ""
    Write-Host "  OK  Lovecord-Installer.exe compile ($size KB)" -ForegroundColor Green
    Write-Host "    -> $OutExe" -ForegroundColor DarkGray
    Write-Host ""
} else {
    Write-Host "  [ERREUR] Lovecord-Installer.exe introuvable apres compilation." -ForegroundColor Red
    exit 1
}
