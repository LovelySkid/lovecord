...
let LovecordBadges = {} as Record<string, Array<{ icon: string; placeholder: string; uuid: string; }>>;
let lovecordBadges = {} as Record<string, Array<Record<"tooltip" | "badge", string>>>;
...
async function loadAllBadges(noCache = false) {
    ...
    const lovecordBadges = await loadBadges(`https://api.${domain}/badges`, noCache).catch(() => ({}));
    const lovecordBadges = await loadBadges("https://raw.githubusercontent.com/ImHisako/ImHisako/refs/heads/main/Images/badges.json", noCache).catch(() => ({}));

    DonorBadges = vencordBadges;
    EquicordDonorBadges = equicordBadges;
    LovecordBadges = lovecordBadges;
    lovecordBadges = lovecordBadges;
}
...
export default definePlugin({
    ...
    getLovecordBadges(userId: string) {
        try {
            const userBadges = LovecordBadges[userId];
            ...
            return userBadges
                ...
                .map(badge => ({
                    iconSrc: badge.icon,
                    description: badge.placeholder ?? "Lovecord Badge",
                    position: BadgePosition.START,
                    props: {
                        style: {
                            borderRadius: "50%",
                    ...
                    onClick() {
                        return GenericBadgeModal(badge, "Lovecord");
                    }
                } satisfies ProfileBadge));
        } catch (e) {
            console.error("[BadgeAPI] Error processing lovecord badges for", userId, e);
            return [];
        }
    },

    getlovecordBadges(userId: string) {
        return lovecordBadges[userId]?.map(badge => ({
            iconSrc: badge.badge,
            description: badge.tooltip,
            position: BadgePosition.START,
            props: {
