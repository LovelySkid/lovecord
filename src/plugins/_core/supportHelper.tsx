...
import { platformName } from "@lovecordplugins/equicordHelper/utils";
...
export default definePlugin({
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        // Groupe 2 : Message "Lovecord developers"
        const lovecordGroupIds = [
        ...
        const isLovecordGroup = lovecordGroupIds.includes(userId);

        if (!isOriginalDev && !isLovecordGroup) return null;
        if (RelationshipStore.isFriend(userId) || isAnyPluginDev(UserStore.getCurrentUser()?.id)) return null;

        const developerText = isOriginalDev ? "Equicord & Vencord plugin developers" : "Lovecord developers";
