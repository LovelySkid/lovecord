...
import { openModal } from "@Lovecord/types/utils";
import { onceReady } from "@Lovecord/types/webpack";
import { VoiceActions } from "@Lovecord/types/webpack/common";
...
import type SettingsPlugin from "@Lovecord/types/plugins/_core/settings";
import { VesktopSettingsIcon } from "shared/icons";

VesktopLogger.log("read if cute :3");
VesktopLogger.log(`Lovecord v${VesktopNative.app.getVersion()}`);

const { customEntries, customSections } = Vencord.Plugins.plugins.Settings as any as typeof SettingsPlugin;

customEntries.push({
    key: "Lovecord_Lovecord_settings",
    title: "Lovecord Settings",
    Component: SettingsUi,
    Icon: VesktopSettingsIcon
});

customSections.push(() => ({
    section: "LovecordSettings",
    label: "Lovecord Settings",
    element: SettingsUi,
    className: "vc-Lovecord-settings"
}));
...
const SECURITY_MODAL_KEY = "lovecord_security_modal_seen";
onceReady.then(() => {
