...
handleSync(IpcEvents.GET_GIT_HASH, () => Lovecord_GIT_HASH);
...
// Handler pour VencordNative.lovecord.relaunch() — utilisé par les boutons Restart dans les settings plugins
...
handleSync(IpcEvents.IS_USING_CUSTOM_VENCORD_DIR, () => !!State.store.LovecordDir);
handle(IpcEvents.SHOW_CUSTOM_VENCORD_DIR, async () => {
    const { LovecordDir } = State.store;
    if (!LovecordDir) return;

    const stats = await stat(LovecordDir);
    if (!stats.isDirectory()) return;

    shell.openPath(LovecordDir);
});
...
handle(IpcEvents.SELECT_VENCORD_DIR, async (_e, value?: null) => {
    if (value === null) {
        delete State.store.LovecordDir;
        return "ok";
    }
