...
const options = {
    ...
    repair: {
        type: "boolean",
        short: "r",
        description: "Re-download Lovecord and restart"
    }
...
const extraOptions = {
    ...
    "ozone-platform": {
        hidden: process.platform !== "linux",
        type: "string",
        description: "Whether to run Lovecord in Wayland or X11 (XWayland)",
        options: ["x11", "wayland"]
    }
} satisfies Record<string, Option>;
...
export async function checkCommandLineForRepair() {
    ...
    if (State.store.LovecordDir) {
        console.error("Cannot repair: using custom Lovecord directory. Remove it in settings first.");
        process.exit(1);
    }

    console.log("Repairing Lovecord...");
...
export function checkCommandLineForHelpOrVersion() {
    ...
    if (version) {
        console.log(`Lovecord v${app.getVersion()}`);
        app.exit(0);
    }

    if (help) {
        const base = stripIndent`
            Lovecord v${app.getVersion()}
...
function checkCommandLineForToggleCommands() {
    ...
    console.error("Lovecord is not running. Toggle commands require a running instance.");
    app.exit(1);
}
...
function checkForSecondInstance() {
    ...
    if (!app.requestSingleInstanceLock({ IS_DEV })) {
        if (IS_DEV) {
            console.log("Lovecord is already running. Quitting previous instance...");
            return;
        } else {
            console.log("Lovecord is already running. Quitting...");
            app.exit(0);
        }
    }
