...
import { openLovecordModal } from "@lovecordplugins/compactMode/LovecordModal";
    ...
    return (
        <Tooltip text={tooltip ?? ""} position={position} shouldShow={tooltip != null}>
            {({ onMouseEnter, onMouseLeave }) => (
                <Clickable
                    {...{ innerRef: ref } as any}
                    className={classes(HeaderBarClasses.clickable, "lovecord-header-btn", className)}
...
try { _stealthActive = localStorage.getItem("Lovecord_stealthMode") === "1"; } catch { }
...
function persistStealth(v: boolean) {
    try { v ? localStorage.setItem("Lovecord_stealthMode", "1") : localStorage.removeItem("Lovecord_stealthMode"); } catch { }
}

const NON_REACT_SELECTORS = [
    "#lovecord-titlebar-btn",
    "#lovecord-titlebar-link-style",
    ".nai-nav-item",
];
...
export function syncStealthBodyClass() {
    try { if (_stealthActive) document.body?.classList.add("lovecord-stealth"); else document.body?.classList.remove("lovecord-stealth"); } catch { }
...
if (_stealthActive) {
    ...
    try { document.body?.classList.add("lovecord-stealth"); } catch { }
}
...
try {
    ...
    window.addEventListener("lovecord-stealth-change", () => {
...
export function _notifyStealthChange() {
    ...
    window.dispatchEvent(new Event("lovecord-stealth-change"));
}
...
try { _compactActive = localStorage.getItem("Lovecord_compactMode") === "1"; } catch { }
...
function persistCompact(v: boolean) {
    try { v ? localStorage.setItem("Lovecord_compactMode", "1") : localStorage.removeItem("Lovecord_compactMode"); } catch { }
}

export function syncCompactBodyClass() {
    try {
        const stored = localStorage.getItem("Lovecord_compactMode");
    ...
    try {
        if (_compactActive) {
            document.body?.classList.add("lovecord-compact");
        } else {
            document.body?.classList.remove("lovecord-compact");
        }
    } catch { }
...
export function toggleCompactMode() {
    ...
    _notifyCompactChange();
    try { if (_compactActive) document.body?.classList.add("lovecord-compact"); else document.body?.classList.remove("lovecord-compact"); } catch { }
    return _compactActive;
}

if (_compactActive) {
    try { document.body?.classList.add("lovecord-compact"); } catch { }
}

export const compactListeners = new Set<() => void>();
export function _notifyCompactChange() {
    compactListeners.forEach(fn => fn());
    window.dispatchEvent(new Event("lovecord-compact-change"));
}
...
function CompactSettingsPopout({ closePopout }: { closePopout: () => void; }) {
    ...
    useEffect(() => {
        ...
        window.addEventListener("lovecord-compact-change", listener);
        window.addEventListener("lovecord-stealth-change", listener);
        return () => {
            compactListeners.delete(listener);
            stealthListeners.delete(listener);
            window.removeEventListener("lovecord-compact-change", listener);
            window.removeEventListener("lovecord-stealth-change", listener);
        };
    }, []);
        ...
        <div className="nc-settings-popout">
            ...
            <div className="nc-settings-popout-row" onClick={() => toggleStealthMode()}>
                <div className="nc-settings-popout-row-info">
                    ...
                    <div className="nc-settings-popout-row-desc">Hide all Lovecord UI elements</div>
                </div>
...
function CompactHeaderBarToggle() {
    ...
    return (
        <div style={{ display: "flex", alignItems: "center" }}>
            ...
            <HeaderBarButton
                icon={GearIcon}
                tooltip="Lovecord Settings"
                onClick={() => openLovecordModal()}
            />
        </div>
    );
}
...
export function _addHeaderBarButtons() {
    return [
        <style key="lovecord-headerbar-style">{`
            .lovecord-header-btn {
                display: flex;
                align-items: center;
                margin: 0 2px;
