...
export type BadgeSource = "vencord" | "equicord" | "lovecord" | "globalbadges" | "lovecord";
...
export async function loadOwnHiddenBadgeSources(userId: string) {
    ...
    try {
        const localData = localStorage.getItem("lovecord_hidden_badges");
        if (localData) {
    ...
    try {
        ...
        if (result?.config?.settings?.hidden !== undefined) {
            ...
            localStorage.setItem("lovecord_hidden_badges", JSON.stringify(hidden));
        }
    } catch (e) {
