...
const API_BASE      = `${GITEA_BASE}/api/v1/repos/lovecord/lovecord`;
const REPO_URL      = `${GITEA_BASE}/lovecord/lovecord`;
declare const VERSION: string;
const CURRENT_VERSION = `v${VERSION}`;
const ZIP_FILE = "lovecord-dist.zip";
...
async function applyUpdates(): Promise<boolean> {
    ...
    try {
        ...
        // Save zip to temp
        const zipPath = join(app.getPath("temp"), `lovecord-update-${Date.now()}.zip`);
        ...
        const tmpExtract = join(app.getPath("temp"), `lovecord-extract-${Date.now()}`);
