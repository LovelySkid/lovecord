...
export const DATA_DIR = process.env.LOVECORD_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "LovecordData", suffix)
        : join(app.getPath("userData"), "..", "Lovecord", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[Lovecord] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
