...
console.log("[Lovecord] Starting up...");
...
if (!IS_VANILLA) {
    ...
    class BrowserWindow extends electron.BrowserWindow {
        constructor(options: BrowserWindowConstructorOptions) {
            // On n'injecte le preload Lovecord QUE dans les fenêtres Discord/Lovecord légitimes.
            ...
            // le titre est une fenêtre Lovecord/Equicord/Discord connue.
            const KNOWN_TITLES = /^(Discord|Vesktop|Equibop)$|^(Lovecord|Equicord)/;
            ...
            if (options?.webPreferences?.preload && (isTrustedTitle || isVBCable || preloadIsOurs)) {
                ...
                if (process.platform === "win32" && winMaterial && winMaterial !== "none") {
                    try {
                        ...
                        if (!applied) {
                            console.warn("[Lovecord] No background material API available on this system");
                        }
                    } catch (e) {
                        console.error("[Lovecord] setBackgroundMaterial failed:", e);
                    }
                }
                ...
                // est géré exclusivement par lovecord-index.js (app.on("web-contents-created"))
    ...
    process.env.DATA_DIR = join(app.getPath("userData"), "..", "Lovecord");
        ...
        (electron.ipcMain as any).handle = function(channel: string, listener: any) {
            ...
            try {
                ...
                if (e?.message?.includes?.("Attempted to register a second handler")) {
                    console.warn(`[Lovecord] Ignored duplicate IPC handler for '${channel}'`);
                    return;
                }
                throw e;
            }
    ...
    console.log("[Lovecord] Running in vanilla mode. Not loading Lovecord");
}

console.log("[Lovecord] Loading original Discord app.asar");
