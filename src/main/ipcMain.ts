...
import { makeLinksOpenExternally } from "../lovecord/main/utils/makeLinksOpenExternally";
...
/**
 * Validates that an IPC event originates from a trusted renderer frame.
 * Accepts frames loaded from:
 *   - The app's own file:// origin (patcher / preload)
 *   - discord.com and discordapp.com over HTTPS
 * Rejects data-URI frames (could be injected by third-party content).
 */
export function validateSender(event: Electron.IpcMainInvokeEvent): boolean {
    ...
    // Allow frames loaded directly from the Electron app bundle
    ...
    // Reject data-URI frames — these are never a legitimate IPC origin
    if (url.startsWith("data:")) return false;

    // Allow Discord's own HTTPS origins
    try {
...
// Lazily import child_process once at module load — avoids repeated synchronous require() calls
import { execFile, spawn as spawnChild } from "child_process";
import { mkdtemp, writeFile as writeFileAsync, rm } from "fs/promises";
import { tmpdir } from "os";

/**
 * Verifies the Authenticode digital signature of a Windows PE file.
 * Always returns `true` on non-Windows platforms (no-op).
 * Uses PowerShell's Get-AuthenticodeSignature which is available on all
 * Windows versions ≥ 8.1 without additional dependencies.
 */
    ...
    return new Promise<boolean>(resolve => {
        execFile(
            ...
            [
                ...
                "-Command",
                // Single-quote-escape the path to prevent injection via file names
                `(Get-AuthenticodeSignature -FilePath '${filePath.replace(/'/g, "''")}').Status`
            ],
            (error, stdout) => resolve(!error && stdout.trim() === "Valid")
        );
    });
}

/**
 * Executes an arbitrary PowerShell script via a temp file on Windows.
 * The script is written with a BOM (UTF-8 with BOM) to ensure PowerShell
 * interprets special characters correctly.
 * The temp directory is always deleted in the finally block.
 */
async function runPowershellScript(psScript: string): Promise<void> {
    if (process.platform !== "win32") return;

    // Create a dedicated temp directory per invocation — prevents races between
    // concurrent calls when multiple IPC handlers fire simultaneously.
    const tempDir = await mkdtemp(join(tmpdir(), "lovecord-ps-"));
    const tempFile = join(tempDir, "script.ps1");
    try {
        // UTF-8 BOM prefix (\uFEFF) ensures PowerShell reads non-ASCII chars correctly
        await writeFileAsync(tempFile, "\uFEFF" + psScript, "utf8");
...
// ── WorldBomb IPC handlers removed ───────────────────────────────────────────
// The native keyboard/mouse automation IPC surface (WORLD_BOMB_TYPE,
// WORLD_BOMB_PRESS_ENTER, WORLD_BOMB_PRESS_BACKSPACE, WORLD_BOMB_CLICK,
// WORLD_BOMB_SEQUENCE, WORLD_BOMB_GET_CURSOR_POS, WORLD_BOMB_OPEN_WINDOW)
// has been permanently removed. The wordBomb plugin now operates as a
// client-side word-suggestion helper with manual user input only.
// ─────────────────────────────────────────────────────────────────────────────
...
ipcMain.handle(IpcEvents.OPEN_MONACO_EDITOR, async (event) => {
    ...
    monacoWin = new BrowserWindow({
        title: "Lovecord QuickCSS Editor",
...
const OFFICIAL_UPDATE_URL = `https://git.${domain}/lovecord/lovecord/releases/download/latest/Lovecord-Installer.exe`;

ipcMain.handle(IpcEvents.LOVECORD_DOWNLOAD_AND_RUN, async (event, url: string) => {
    if (!validateSender(event)) throw new Error("Unauthorized IPC invocation");
    ...
    const tmpPath = path.join(os.tmpdir(), "LovecordUpdate-Setup.exe");
    ...
    const { response } = await dialog.showMessageBox({
        ...
        defaultId: 0,
        title: "Lovecord Update",
        message: "A Lovecord update is available.",
...
ipcMain.handle(IpcEvents.INSTALL_VB_CABLE, async (event) => {
    ...
    const tmpDir = path.join(os.tmpdir(), "Lovecord-VBCable");
    ...
    try {
        ...
        console.error("[Lovecord] VBCable install failed:", err);
