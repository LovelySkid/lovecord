...
/**
 * Initializes the Lovecord system tray icon.
 ...
 */
export function initLovecordTray(injectorPath: string) {
    ...
    app.whenReady().then(() => {
        try {
            ...
            const trayMenu = Menu.buildFromTemplate([
                ...
                { type: "separator" },
                {
                    label: "Uninstall Lovecord",
                    click() {
                        const uninstallerCandidates = [
                            join(process.env.LOCALAPPDATA || "", "Programs", "lovecord", "Uninstall Lovecord.exe"),
                            join(process.env.LOCALAPPDATA || "", "Programs", "Lovecord", "Uninstall Lovecord.exe"),
                            join(process.env.PROGRAMFILES || "", "Lovecord", "Uninstall Lovecord.exe"),
                        ];
            ...
            console.log("[Lovecord] Tray created ✓", iconPath ?? "(icon from exe)");
        } catch (e) {
            console.error("[Lovecord] Failed to create tray:", e);
        }
    });
}
