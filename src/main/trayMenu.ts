...
function createLovecordMenuItems(): MenuItemConstructorOptions[] {
    return [
        {
            label: "Lovecord",
            submenu: [
                {
                    label: "About Lovecord",
                    click: () => openAboutWindow()
                },
                {
                    label: cachedUpdateAvailable ? "Update Lovecord" : "Check for Updates",
...
export function patchTrayMenu(): void {
    // No longer patching tray menu — Lovecord manages its own tray in patcher.ts
}
