...
const LOVECORD_RELEASES_REPO = "lovecord/lovecord";
const LOVECORD_REPO_URL = `https://source.${domain}/${LOVECORD_RELEASES_REPO}`;
...
export { LOVECORD_REPO_URL };

export async function getCommitsSinceLastSeen(
    repoUrl: string,
): Promise<ChangelogEntry[]> {
    // Toujours utiliser le repo Lovecord, ignorer le repoUrl d'Equicord
    return fetchCommitsBetween(LOVECORD_RELEASES_REPO, "HEAD~10", "HEAD").catch(() => []);
}
