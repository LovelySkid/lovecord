...
function LovecordTabIcon() {
    return <img src={LOVECORD_TAB_ICON} alt="Lovecord" style={{ width: 18, height: 18, borderRadius: 4 }} />;
}
...
const makeCategoryOptions = (othersCount?: number) => [
    ...
    { label: "Lovecord", value: SearchStatus.LOVECORD },
...
export const ExcludedReasons: Record<"web" | "discordDesktop" | "vesktop" | "equibop" | "desktop" | "dev", string> = {
    ...
    dev: "Developer version of Lovecord"
};
...
export default function PluginSettings({ premiumOnly = false }: PluginSettingsProps) {
    ...
    const [searchValue, setSearchValue] = useState({ value: "", status: SearchStatus.LOVECORD });
    ...
    const pluginFilter = useCallback((plugin: typeof Plugins[keyof typeof Plugins], newPluginsSet: Set<string> | null) => {
        ...
        switch (status) {
            ...
            case SearchStatus.ENABLED:
                if (!enabled) return false;
                break;
            case SearchStatus.LOVECORD:
                if (!pluginMeta?.folderName?.startsWith("src/lovecordplugins/")) return false;
                break;
            case SearchStatus.OTHERS:
                if (pluginMeta?.folderName?.startsWith("src/lovecordplugins/") || pluginMeta?.folderName?.startsWith("src/plugins/_")) return false;
        ...
        if ((search.includes("lovecord") || search.includes("illegal")) && islovecordPartner) {
            return true;
        }
    ...
    // Only filter/categorize plugin DATA here — no JSX created yet
        const { lovecordData, othersData, requiredData } = useMemo(() => {
        const lovecordData: typeof sortedPlugins = [];
        ...
        for (const p of sortedPlugins) {
            ...
            if (isRequired) {
                ...
                if (folderName.startsWith("src/lovecordplugins/")) {
                    lovecordData.push(p);
                } else {
                    othersData.push(p);
                }
        ...
        lovecordData.sort(byLikes);
        othersData.sort(byLikes);

        return { lovecordData, othersData, requiredData };
    }, [sortedPlugins, searchValue, newPluginsSet, depMap, pluginFilter, ratings]);

    const allDataLength = lovecordData.length + othersData.length;
    ...
    const lovecordVisibleData = lovecordData.slice(0, Math.min(visibleCount, lovecordData.length));
    const othersVisibleData = othersData.slice(0, Math.max(0, visibleCount - lovecordData.length));
    ...
    const makeRequiredCard = (p: typeof sortedPlugins[number]) => {
        const tooltipText = p.required || !depMap[p.name]
            ? "This plugin is required for Lovecord to function."
    ...
    const lovecordPlugins = lovecordVisibleData.map(makeCard);
    const othersVisible = othersVisibleData.map(makeCard);
    const requiredPlugins = requiredData.map(makeRequiredCard);

    const totalLovecordPlugins = React.useMemo(() => {
        return Object.values(Plugins).filter(p => PluginMeta[p.name]?.folderName?.startsWith("src/lovecordplugins/")).length;
    }, []);

    const totalOtherPlugins = React.useMemo(() => {
        const isLovecordPlugin = (p: string) => PluginMeta[p]?.folderName?.startsWith("src/lovecordplugins/");
        const isCorePlugin = (p: string) => PluginMeta[p]?.folderName?.startsWith("src/plugins/_");
        return Object.values(Plugins).filter(p => !isLovecordPlugin(p.name) && !isCorePlugin(p.name)).length;
    }, []);

    // Category-aware stats for the "ENABLED PLUGINS" card: reflects whichever tab
    // (LOVECORD / OTHERS / all) is currently selected, instead of always being global.
    const categoryStats = useMemo(() => {
        ...
        if (searchValue.status === SearchStatus.LOVECORD) {
            plugins = plugins.filter(isLovecordPlugin);
        } else if (searchValue.status === SearchStatus.OTHERS) {
            plugins = plugins.filter(p => !isLovecordPlugin(p));
            ...
            plugins = plugins.filter(p => isLovecordPlugin(p) && ratings[p]?.likedByMe);
        }
        ...
        <SettingsTab>
                ...
                {!premiumOnly && (
                    <div className={cl("ecosystem-banner")}>
                        <div className={cl("ecosystem-banner-text")}>
                            ...
                            <Paragraph>{t("Manage your Lovecord and community plugins here. Enable, disable, and configure them to your liking.")}</Paragraph>
                        </div>
                            ...
                            onClick={() => {
                                ...
                                if (native?.process?.env) {
                                    ...
                                    if (home) {
                                        ...
                                        const folderPath = isWindows ? `${home}\\Documents\\Lovecord\\userplugins` : `${home}/Documents/Lovecord/userplugins`;
                ...
                <div className={classes(Margins.bottom20, cl("filter-controls"))}>
                    ...
                    <div className={cl("filter-buttons")} style={{ minWidth: 220 }}>
                        <SearchableSelect
                            ...
                            value={makeCategoryOptions(totalOtherPlugins).find(o => o.value === searchValue.status)?.value ?? SearchStatus.LOVECORD}
                            onChange={(v: any) => {
                            ...
                            renderOptionPrefix={(o: any) => {
                                if (o?.value === SearchStatus.LOVECORD) return <LovecordTabIcon />;
            ...
            {premiumOnly ? (
                <>
                    ...
                    {lovecordData.length || othersData.length
                        ? (
                            <div className={cl("grid")}>
                                {[...lovecordPlugins, ...othersVisible].length
                                    ? [...lovecordPlugins, ...othersVisible]
                ...
                <>
                    {lovecordData.length > 0 && searchValue.status === SearchStatus.LOVECORD && (
                        <div className={cl("grid")}>
                            {lovecordPlugins}
                        </div>
                    )}
                    ...
                    {searchValue.status === SearchStatus.USER_PLUGINS && (
                        <>
                            ...
                            {lovecordPlugins.length > 0 || othersVisible.length > 0 ? (
                                <div className={cl("grid")}>
                                    {[...lovecordPlugins, ...othersVisible]}
                                </div>
                            ) : (
                                <div style={{ textAlign: "center", padding: "48px 16px", color: "var(--text-muted)" }}>
                                    ...
                                    <div style={{ fontSize: 13 }}>{t("Add .tsx files to your")} <code>Documents/Lovecord/userplugins/</code> {t("folder and rebuild.")}</div>
                                </div>
                            )}
                        </>
                    )}

                    {(searchValue.status !== SearchStatus.LOVECORD && searchValue.status !== SearchStatus.OTHERS && searchValue.status !== SearchStatus.USER_PLUGINS) && (
                        <div className={cl("grid")}>
                            {lovecordPlugins}
                            {othersVisible}
                        </div>
                    ...
                    {lovecordData.length === 0 && othersData.length === 0 && searchValue.status === SearchStatus.LIKED_PLUGINS && (
                        <div style={{ textAlign: "center", padding: "48px 16px", color: "var(--text-muted)" }}>
                            ...
                            <div style={{ fontSize: 13 }}>{t("Like a Lovecord plugin from its card to see it here.")}</div>
                        </div>
                    )}

                    {lovecordData.length === 0 && othersData.length === 0 && searchValue.status !== SearchStatus.USER_PLUGINS && searchValue.status !== SearchStatus.LIKED_PLUGINS && (
                        <ExcludedPluginsList search={search} />
                    )}
