...
export function PluginCard({ plugin, disabled, onRestartNeeded, onMouseEnter, onMouseLeave, isNew, hasTutorial }: PluginCardProps) {
    ...
    const openTutorialVideo = (e: React.MouseEvent) => {
        ...
        const videoUrl = `https://git.${domain}/lovecord/lovecord-tutorials/raw/branch/main/videos/${encodeURIComponent(videoName)}.mp4`;
        openModal(props => (
    ...
    const isLovecord = !PluginMeta[plugin.name]?.userPlugin;
    const iconType = isLovecord ? "lovecord" : "other";

    // Le système de like ne s'applique qu'aux plugins Lovecord (pas Vencord/Equicord,
    ...
    const isLovecordFolderPlugin = PluginMeta[plugin.name]?.folderName?.startsWith("src/lovecordplugins/") ?? false;
    const canShowLikeBadge = isLovecordFolderPlugin && !plugin.required && !disabled;
        ...
        openModal(props => (
            <ModalRoot {...props} size={ModalSize.SMALL}>
                ...
                <ModalContent style={{ padding: "16px", display: "flex", flexDirection: "column", gap: "16px", alignItems: "center" } as any}>
                    {isLovecord ? (
                        <a href="https://source.lovecord.gg/lovecord/lovecord" target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: "12px", textDecoration: "none", color: "var(--text-normal)", fontSize: "20px", fontWeight: 600 }}>
                            <img src="https://source.lovecord.gg/assets/img/logo.svg" alt="Lovecord" style={{ width: 64, height: 64, borderRadius: "50%" }} />
                            Lovecord
                        </a>
                    ) : (
            ...
            infoButton={
                <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                    {(plugin.name === "DynamicIslande" || plugin.name === "StereoInstaller" || plugin.name === "ClientDiagnostics" || plugin.name === "SecureBookmarks" || plugin.name === "StatusCycler" || plugin.name === "Surveillance" || plugin.name === "MutualScanner") && (
                        <Tooltip text="This plugin comes from our partner (lovecord) then was modified by Lovecord.">
                            {({ onMouseEnter, onMouseLeave }) => (
                                <button
                                    role="button"
                                    ...
                                    onMouseEnter={onMouseEnter}
                                    onMouseLeave={onMouseLeave}
                                    onClick={() => window.open("https://github.com/ImHisako/lovecord", "_blank")}
                                >
