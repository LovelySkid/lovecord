...
import IconsTab from "@lovecordplugins/iconViewer/components/IconsTab";
...
const TABS: TabDef[] = [
    ...
    },
    {
        ...
        description: "Sync Lovecord settings across devices",
    },
    {
        id: "language",
        label: "Language",
        ...
        description: "Change Lovecord's interface language",
    },
];

// ── Modal inner ───────────────────────────────────────────────────────────────

function LovecordModalInner({ onClose }: { onClose: () => void; }) {
    ...
    let TabContent = null;
    switch (activeTab) {
        case "lovecord": TabContent = <VencordTab />; break;
...
function LovecordModal({ modalProps }: { modalProps: ModalProps; }) {
    return (
        <ModalRoot {...modalProps} size={ModalSize.LARGE} className="nc-modal-wrapper">
            <ModalContent className="nc-modal-modal-content">
                <ErrorBoundary message="Failed to render Lovecord Settings modal.">
                    <LovecordModalInner onClose={modalProps.onClose} />
                </ErrorBoundary>
            </ModalContent>
        </ModalRoot>
    );
...
export function openLovecordModal() {
    openModal(modalProps => <LovecordModal modalProps={modalProps} />);
}
