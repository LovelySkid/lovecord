...
import { t, useTranslation } from "../autoTranslateLovecord";
...
function loadPersistLogs() {
    try {
        const s = localStorage.getItem("lovecord_logs");
        if (s) {
...
function savePersistLogs() {
    try {
        ...
        localStorage.setItem("lovecord_logs", JSON.stringify(toSave));
    } catch { }
}
...
function LogRow({ e }: { e: LogEntry; }) {
    ...
    const onDoubleClick = () => {
        ...
        try {
            ...
            console.error("Lovecord Navigation Error:", err);
...
function LogsModal({ rootProps }: { rootProps: any; }) {
    ...
    const saveAsTxt = useCallback(() => {
        try {
            ...
            a.href = url;
            a.download = `lovecord_logs_${new Date().toISOString().slice(0, 10)}.txt`;
            a.click();
...
export default definePlugin({
    ...
    start() {
        ...
        loadPersistLogs();
        addHeaderBarButton("lovecord-event-logs", () => <LogsButton />, 7);
        subscribeToEvents();
    ...
    stop() {
        ...
        removeHeaderBarButton("lovecord-event-logs");
