...
import { t } from "../autoTranslateLovecord";
...
const MI_TOKEN_CACHE_KEY = "lovecord-mi-token-cache";
...
function ContextMenuPortal(props: CtxMenuProps) {
    ...
    const [container] = React.useState(() => {
        const el = document.getElementById("lovecord-mi-ctx-root") ?? document.createElement("div");
        el.id = "lovecord-mi-ctx-root";
...
export default definePlugin({
    ...
    async start() {
        ...
        captureCurrentToken();
        addHeaderBarButton("lovecord-multi-instance", () => <MultiInstanceButton />, 9);
    },

    stop() {
        removeHeaderBarButton("lovecord-multi-instance");
        if (this._fluxUnsub) { this._fluxUnsub(); this._fluxUnsub = null; }
        const root = document.getElementById("lovecord-mi-ctx-root");
        if (root) root.remove();
    },
});
