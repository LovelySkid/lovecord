...
import { registerMediaPermissionsForSession } from "../../lovecord/main/mediaPermissions";
...
//
// Chaque instance tourne dans sa propre session Electron (persist:lovecord-mi-{userId}),
...
const SHARED_SETTINGS_FILE = join(app.getPath("userData"), "lovecord-mi-shared-settings.json");
...
function saveSharedSettings(settings: Record<string, string>): void {
    try {
        ...
        console.warn("[LovecordMI] Failed to save shared settings:", e);
    }
}
...
function createTokenPreload(token: string, sharedSettings: Record<string, string> = {}): string {
    ...
    const dir = join(app.getPath("userData"), "lovecord-mi-preloads");
    ...
    const script = `
// Lovecord MultiInstance — token preload
...
(function() {
    ...
    try {
        ...
        try {
            ...
            console.warn("[LovecordMI] Shared settings seed error:", e);
        }

        // Définit le token dans localStorage
        Object.defineProperty(window, '__lovecord_token', { value: TOKEN, writable: false });
        ...
        console.log("[LovecordMI] Token preload active ✓");
    } catch(e) {
        console.warn("[LovecordMI] Preload error:", e);
    }
})();
`;
...
export async function openInstanceWindow(
    ...
    try {
        ...
        const uniqueAppId = `lovecord.instance.${userId}.${Date.now()}`;
        ...
        const partition = `persist:lovecord-mi-${userId}`;
        ...
        const win = new BrowserWindow({
            ...
            title: `Lovecord [${username || userId}]`,
        ...
        if (process.platform === "win32") {
            try {
                win.setAppDetails({
                    ...
                    relaunchDisplayName: `Lovecord [${username || userId}]`,
                });
            } catch (err) {
                console.warn("[LovecordMI] setAppDetails failed:", err);
            }
        }
            ...
            wc.executeJavaScript(`
                (async () => {
                    ...
                    try {
                        ...
                        const ws = window.__LOVECORD_GW_WS__;
...
// ─────────────────────────────────────────────────────────────────────────────
// Fenetres « groupées » — meme groupe que Lovecord dans la barre des taches
// Principe : on ne touche PAS a setAppDetails => la fenetre herite de l'AppId
// du processus principal (com.lovecord.app), Windows la groupe automatiquement
// ─────────────────────────────────────────────────────────────────────────────
...
export async function arrangeSplit(_: any, userId: string): Promise<void> {
    try {
        ...
        console.error("[LovecordMI] arrangeSplit error:", e);
    }
}
