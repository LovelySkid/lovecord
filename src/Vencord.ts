...
function showGreenUpdateBanner() {
    if (document.getElementById("lovecord-core-updater-root")) return;

    const banner = document.createElement("div");
    banner.id = "lovecord-core-updater-root";
    ...
    titleSpan.textContent = "Lovecord Update Available";
...
// @ts-ignore
window.showLovecordUpdateBanner = showGreenUpdateBanner;
...
function initTrayIpc() {
    ...
    VencordNative.tray.onCheckUpdates(async () => {
        try {
            ...
            if (isOutdated) {
                showNotice("A Lovecord update is available!", "View Update", () => openSettingsTabModal(UpdaterTab!));
            } else {
    ...
    VencordNative.tray.onRepair(async () => {
        try {
            ...
            UpdateLogger.error("Failed to repair Lovecord", err);
        }
    });
    ...
    if (IS_DEV) {
        ...
        if (pendingPatches.length)
            PMLogger.warn(
                ...
                "You are seeing this warning because this is a Development build of Lovecord.",
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    createAndAppendStyle("lovecord-titlebar-position", coreStyleRootNode).textContent = `
        body:not(.lovecord-stealth):not(.lovecord-compact) [class*="title_c38"] {
