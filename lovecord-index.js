...
// ── CRITIQUE : userData = dossier Lovecord pour les settings/plugins
const lovecordData = path.join(app.getPath("appData"), "Lovecord");
app.setPath("userData", lovecordData);

// AppUserModelId unique — Windows reconnaît Lovecord comme app séparée de Discord
...
app.once("ready", () => {
    try {
        ...
        //     Seuls les modules vraiment inutiles pour Lovecord sont bloqués.
        ...
        function patchWebContents(wc) {
            ...
            if (wc._lovecordPatched) return;
            wc._lovecordPatched = true;
            ...
            wc.setWindowOpenHandler(({ url }) => {
                ...
                console.log("[Lovecord][LINK] Ouverture externe:", url);
                return { action: "deny" };
            });
            ...
            wc.on("did-create-window", (childWin) => {
                ...
                if (childWc._lovecordPatched) return;
                childWc._lovecordPatched = true;
                ...
                childWc.on("will-navigate", (event, url) => {
                    if (!isDiscordUrl(url)) {
                        ...
                        shell.openExternal(url).catch(() => {});
                        console.log("[Lovecord][CHILD-NAV] Redirection externe:", url);
                ...
                childWc.on('did-navigate', function(_event, url) {
                    if (!isDiscordUrl(url)) {
                        ...
                        console.log('[Lovecord][CHILD-DID-NAV] Redirection externe apres navigation:', url);
                        try { childWin.close(); } catch (_) {}
                    }
                });
                ...
                childWc.setWindowOpenHandler(({ url }) => {
                    ...
                    shell.openExternal(url).catch(() => {});
                    console.log("[Lovecord][CHILD-LINK] Ouverture externe:", url);
                    return { action: "deny" };
                });
                ...
                childWc.on("did-finish-load", () => {
                    ...
                    if (url && url !== "about:blank" && !isDiscordUrl(url)) {
                        shell.openExternal(url).catch(() => {});
                        console.log("[Lovecord][CHILD-LOAD] Fermeture et redirection:", url);
                        try { childWin.close(); } catch (_) {}
                    }
                });
            });
            ...
            wc.on("will-navigate", (event, url) => {
                ...
                if (url !== currentUrl && !isDiscordUrl(url)) {
                    event.preventDefault();
                    shell.openExternal(url).catch(() => {});
                    console.log("[Lovecord][NAV] Redirection externe:", url);
                }
            });
        }
        ...
        console.log("[Lovecord] Patch liens externes activé sur TOUS les webContents (avec did-create-window) ✓");
        ...
        app.once("browser-window-created", (_, win) => {
            ...
            try {
                ...
                ses.webRequest.onBeforeRequest(
                    ...
                    (details, callback) => {
                        ...
                        if (isBlocked) {
                            ...
                            console.log("[Lovecord] Module bloqué (inutile pour Lovecord):", url.split("/").slice(-2).join("/"));
                ...
                console.log("[Lovecord] Filtre modules 403 activé ✓");
            } catch (e) {
                console.warn("[Lovecord] Impossible d'activer le filtre modules:", e.message);
            }
        });
    } catch (e) {
        console.warn("[Lovecord] FIX modules 403 failed:", e.message);
    }
});
...
try {
    const lsPath = path.join(lovecordData, "Local Storage", "leveldb");
    if (fs.existsSync(lsPath)) {
        ...
        if (corrupted) {
            console.warn("[Lovecord] LevelDB localStorage corrompu détecté — réparation...");
            try { fs.rmSync(lsPath, { recursive: true, force: true }); } catch { }
            console.warn("[Lovecord] LevelDB supprimé — les données localStorage seront récréées");
        }
    }
} catch (e) { console.warn("[Lovecord] LevelDB check failed:", e.message); }

// Modules bundlés dans lovecord-dist/modules/
...
try {
    ...
    if (entries.length > 0) {
        ...
        console.log("[Lovecord] Modules natifs Discord détectés:", discordNativeModulesPath);
    }
} catch (e) {
    console.warn("[Lovecord] Impossible de détecter les modules natifs Discord:", e.message);
}
...
// Priorité aux modules bundlés (portables, dans lovecord-dist/modules/)
...
if (discordNativeModulesPath) {
    ...
    } catch (e) { console.warn("[Lovecord] Erreur lors du scan des modules natifs:", e.message); }
}
try {
...
// parent.paths = []) trouvent quand même les modules natifs Lovecord.
...
console.log("[Lovecord] mainAppDirname:", global.mainAppDirname);
...
try {
    ...
    if (fs.existsSync(nativeModulesDir)) {
        ...
        if (!buildInfo.localModulesRoot) {
            ...
            console.log("[Lovecord] build_info.json patché → localModulesRoot:", nativeModulesDir);
        }
    }
} catch (e) {
    console.warn("[Lovecord] Impossible de patcher build_info.json:", e.message);
}
