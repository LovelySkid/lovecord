...
# ── Vérification ─────────────────────────────────────────────────────────────
if [[ ! -f "release/installer/Lovecord-Installer.exe" ]]; then
    echo ""
    echo " [ERREUR] Lovecord-Installer.exe introuvable apres build."
    exit 1
fi

SIZE=$(stat -c%s "release/installer/Lovecord-Installer.exe" 2>/dev/null \
    || stat -f%z "release/installer/Lovecord-Installer.exe")

echo ""
echo " [OK] Build reussi !"
echo " Fichier : release/installer/Lovecord-Installer.exe  ($SIZE octets)"
echo ""