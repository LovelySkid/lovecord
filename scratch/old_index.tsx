...
function YoutubeHomeModal({ onClose, onPlayVideo }: {
    ...
    const [status, setStatus] = useState("­ƒöÑ Trending");

    async function doSearch(e?: React.FormEvent) {
        e?.preventDefault();
        const q = query.trim();
        if (!q) { setVideos(DEFAULT_VIDEOS); setChannels([]); setChannelInfo(null); setStatus("­ƒöÑ Trending"); return; }
        setLoading(true);
        setStatus("Searching...");
        try {
...
const RPC_APP_ID = "1108588077900898414"; // Shared Lovecord music/media app ID
    ...
    try {
        ...
        FluxDispatcher.dispatch({
            ...
            activity: {
                ...
                name: "Watch Youtube in Lovecord",
                ...
                metadata: {
                    button_urls: [
                        `https://lovecord.gg/watch?v=${p.video.id}`,
                        "https://lovecord.gg",
                    ],
                },
                flags: 1,
