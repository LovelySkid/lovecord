# ==============================================================================
#  Lovecord — Installeur utilisateur (PowerShell autonome)
#  
#  Ce script fait TOUT automatiquement :
#  1. Télécharge EquilotlCli.exe (outil d'injection graphique)
#  2. Télécharge les fichiers Lovecord compilés depuis GitHub
#  3. Lance l'interface graphique pour choisir votre Discord cible
#  4. Injecte Lovecord dans Discord
#
...
function Write-Banner {
    ...
    Write-Host "  ║          LOVECORD  INSTALLER            ║" -ForegroundColor Cyan
...
if (Test-Path $EquilotlExe) {
    ...
    try {
        $head = Invoke-WebRequest -Uri $EquilotlUrl -Method Head -UseBasicParsing `
            -Headers @{ "User-Agent" = "Lovecord-Installer/2.0" }
...
# ── [2/3] Télécharger les fichiers Lovecord ──────────────────────────────────
Write-Step 2 3 "Téléchargement des fichiers Lovecord depuis GitHub..."

try {
    $apiUrl   = "https://api.github.com/repos/$LovecordRepo/releases/latest"
    $release  = Invoke-RestMethod -Uri $apiUrl -UseBasicParsing `
        -Headers @{ "User-Agent" = "Lovecord-Installer/2.0"; "Accept" = "application/vnd.github.v3+json" }

    $version  = $release.tag_name
    $distAsset = $release.assets | Where-Object { $_.name -eq "lovecord-dist.zip" } | Select-Object -First 1

    if (-not $distAsset) {
        Write-Fail "Fichier 'lovecord-dist.zip' introuvable dans la release $version.`n           Contactez le support Lovecord."
    }

    Write-Host "          Version : $version" -ForegroundColor DarkGray
    Write-Host "          Téléchargement en cours..." -ForegroundColor DarkGray

    $zipPath = Join-Path $InstallDir "lovecord-dist.zip"
    ...
    Write-OK "Lovecord $version prêt à être injecté !"
} catch {
    Write-Fail "Échec du téléchargement Lovecord.`n           Détail : $_"
}
...
Write-Host "          │  Sélectionnez le Discord où injecter Lovecord. │" -ForegroundColor DarkCyan
Write-Host "          └─────────────────────────────────────────────────┘" -ForegroundColor DarkCyan
Write-Host ""
...
Write-Host "  ║  Lovecord installé avec succès !                    ║" -ForegroundColor Green
Write-Host "  ║                                                      ║" -ForegroundColor Green
Write-Host "  ║  → Redémarrez Discord pour appliquer Lovecord.      ║" -ForegroundColor Green
Write-Host "  ╚══════════════════════════════════════════════════════╝" -ForegroundColor Green
Write-Host ""
Write-Host "  Pour désinstaller : exécutez lovecord-uninstall.bat" -ForegroundColor DarkGray
Write-Host ""
